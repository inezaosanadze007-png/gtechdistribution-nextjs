...
export default function QualitySection({ dict }: { dict: Dictionary }) {
  return (
    <section id="quality" className="border-t border-line bg-band text-ink scroll-mt-20">
      <div className="max-w-[1180px] mx-auto px-6 py-16">
        <Reveal>
          <div className="mb-10 max-w-[560px]">
            ...
            <p className="text-inkSoft text-[15px]">{dict.quality.sub}</p>
          </div>
        </Reveal>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-px bg-line rounded-lg overflow-hidden">
          {dict.quality.items.map((it, i) => (
            <Reveal key={i} delay={i * 0.08}>
              <div className="bg-band px-[18px] py-6 h-full">
                ...
                <div className="text-[13px] text-inkSoft leading-relaxed">{it.d}</div>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal delay={0.3}>
          <div className="flex flex-wrap items-center gap-7 mt-6 border border-line rounded-lg px-[22px] py-[18px]">
            <div className="font-mono text-[10.5px] text-accent tracking-[0.06em]">
              {dict.quality.sample.label}
            </div>
            <div className="font-mono text-[13px]">{dict.quality.sample.device}</div>
            <div className="font-mono text-[13px] text-ink/85">{dict.quality.sample.read}</div>
            <div className="font-mono text-[13px] text-ink/85">{dict.quality.sample.write}</div>
            <div className="font-mono text-[13px] text-ink/85">{dict.quality.sample.health}</div>
          </div>
        </Reveal>
      </div>
    </section>
