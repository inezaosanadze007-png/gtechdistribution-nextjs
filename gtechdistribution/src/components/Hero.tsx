...
export default function Hero({ dict }: { dict: Dictionary }) {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-deep text-deepInk">
      ...
      <div className="max-w-[1180px] mx-auto px-6 pt-[104px] pb-[88px] md:pt-[136px] md:pb-[116px]">
        <Reveal>
          <div className="max-w-[620px]">
            <div className="font-mono text-xs tracking-[0.14em] uppercase text-ember mb-5">
              {dict.hero.eyebrow}
            </div>
            <h1 className="font-display font-bold text-[38px] md:text-[52px] leading-[1.08] tracking-tight mb-5">
              {dict.hero.title}
            </h1>
            <p className="text-deepInkSoft text-base md:text-[17px] leading-relaxed max-w-[520px] mb-9">
              {dict.hero.sub}
            </p>
            <div className="flex flex-wrap gap-3 mb-12">
              <a
                href="#products-teaser"
                className="cta-btn bg-trace text-deep rounded-md px-[22px] py-[13px] text-sm font-semibold"
              >
                {dict.cta.viewProducts}
              </a>
              <a
                href="../contact"
                className="cta-btn bg-deep/40 backdrop-blur border border-deepLine hover:border-trace text-deepInk rounded-md px-[22px] py-[13px] text-sm font-semibold"
              >
                {dict.cta.quote}
              </a>
            </div>
            <div className="flex flex-wrap gap-x-10 gap-y-5 border-t border-deepLine pt-6">
              {stats.map(([v, l], i) => (
                <div key={i}>
                  <div className="font-mono font-medium text-[15px] text-trace">{v}</div>
                  <div className="text-xs text-deepInkSoft mt-1">{l}</div>
                </div>
              ))}
            </div>
          </div>
