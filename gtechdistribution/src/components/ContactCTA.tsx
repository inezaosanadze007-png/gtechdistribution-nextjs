...
export default function ContactCTA({ dict, lang }: { dict: Dictionary; lang: Locale }) {
  return (
    <section className="bg-deep text-deepInk">
      <div className="max-w-[720px] mx-auto px-6 py-[72px] text-center">
        <Reveal>
          ...
          <p className="text-deepInkSoft text-[15px] mb-7">{dict.contact.sub}</p>
          <Link
            href={`/${lang}/contact`}
            className="cta-btn inline-block bg-trace text-deep rounded-md px-7 py-[13px] text-sm font-semibold"
          >
            {dict.contact.button}
          </Link>
        </Reveal>
